...
class AuthorListViewTest(TestCase):
    ...
    def setUpTestData(cls):
        ...
        for author_num in range(number_of_authors):
            Author.objects.create(first_name='Jessy %s' % author_num, last_name= 'Yang %s' % author_num,)
    ...
    def test_view_pagination_is_two(self):
        resp = self.client.get(reverse('authors'))
        self.assertEqual(resp.status_code, 200)
    ...
    def test_lists_all_authors(self):
        # Get 7 th page and confirm it has (exactly) remaining 1 items
...
class LoanedBooksByUserListViewTest(TestCase):
    ...
    def test_only_borrowed_books_in_list(self):
        ...
        # Check our user is login 
        self.assertEqual(str(resp.context['user']), 'testuser1')

        # Check we got a response success
        self.assertEqual(resp.status_code, 200)

        # Check we don't have anybooks

        self.assertTrue('bookinstance_list' in resp.context)
        self.assertEqual(len(resp.context['bookinstance_list']), 0)

        # Now change all books to be on loaned
